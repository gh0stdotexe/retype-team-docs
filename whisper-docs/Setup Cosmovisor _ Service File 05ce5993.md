...
```shell
# empower cosmovisor daemon
export DAEMON_NAME=empowerd
export DAEMON_HOME=$HOME/.empowerchain
```
...
sudo chown -R whispernode:whispernode ~/.empowerchain/cosmovisor
```
...
Then use the path returned to copy it to the directory Cosmovisor expects. Let's assume the previous command returned `/home/your-user/go/bin/empowerd`:

```shell
cp $GOPATH/bin/empowerd ~/.empowerchain/cosmovisor/genesis/bin
```
...
Commands sent to Cosmovisor are sent to the underlying binary. For example, `cosmovisor version` is the same as typing `empowerd version`.

Nevertheless, just as we would manage `empowerd` using a process manager, we would like to make sure Cosmovisor is automatically restarted if something happens, for example, an error or reboot.
...
```shell
[Unit]
Description=Empower Testnet cosmovisor
...
Environment="DAEMON_HOME=/home/whispernode/.empowerchain"
