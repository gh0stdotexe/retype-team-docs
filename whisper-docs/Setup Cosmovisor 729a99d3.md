...
```shell
# cosmovisor - Cerberus
export DAEMON_NAME=migalood
export DAEMON_HOME=$HOME/.migalood
```
...
```shell
mkdir -p ~/.migalood/cosmovisor/genesis/bin 
mkdir -p ~/.migalood/cosmovisor/upgrades

# Change Ownership of newly created folders
sudo chown -R whispernode:whispernode ~/.migalood/cosmovisor
```
...
```shell
cp $GOPATH/bin/migalood ~/.migalood/cosmovisor/genesis/bin
```
...
```shell
[Unit]
Description=cosmovisor-10 | WhiteWhale (migalood)
After=network-online.target
[Service]
User=whispernode
ExecStart=/home/whispernode/go/bin/cosmovisor-10 start
Restart=always
RestartSec=3
LimitNOFILE=4096
Environment="DAEMON_NAME=migalood"
Environment="DAEMON_HOME=/home/whispernode/.migalood"
