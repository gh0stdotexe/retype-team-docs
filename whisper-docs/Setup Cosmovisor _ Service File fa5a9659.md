...
Commands sent to Cosmovisor are sent to the underlying binary. For example, `cosmovisor version` is the same as typing `qusarnoded version`.
...
```shell
sudo nano /etc/systemd/system/cosmovisor-12.service
```
...
```shell
[Unit]
Description=cosmovisor-12 Quasarnoded
After=network-online.target
[Service]
User=whispernode
ExecStart=/home/whispernode/go/bin/cosmovisor-12 start
Restart=always
RestartSec=3
LimitNOFILE=4096
