...
sudo chown -R whispernode:whispernode ~/.teritorid/cosmovisor
```
...
Then use the path returned to copy it to the directory Cosmovisor expects. Let's assume the previous command returned `/home/your-user/go/bin/teritorid`:

```shell
cp $GOPATH/bin/teritorid ~/.teritorid/cosmovisor/genesis/bin
```
...
```shell
[Unit]
Description=Teritori cosmovisor
...
Environment="DAEMON_HOME=/home/whispernode/.teritorid"
