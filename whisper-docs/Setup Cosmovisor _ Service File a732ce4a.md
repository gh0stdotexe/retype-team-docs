...
```shell
export DAEMON_NAME_1=pylonsd
export DAEMON_HOME_1=$HOME/.pylons
```
...
```shell
mkdir -p ~/.pylons/cosmovisor/genesis/bin
mkdir -p ~/.pylons/cosmovisor/upgrades

# Change Ownership of newly created folders (if needed)
sudo chown -R whispernode:whispernode ~/.pylons/cosmovisor
```
...
Then use the path returned to copy it to the directory Cosmovisor expects. Let's assume the previous command returned `/home/your-user/go/bin/pylonsd`:

```shell
cp $GOPATH/bin/pylonsd ~/.pylons/cosmovisor/genesis/bin
```
...
Commands sent to Cosmovisor are sent to the underlying binary. For example, `cosmovisor version` is the same as typing `pylonsd version`.

Nevertheless, just as we would manage `pylonsd` using a process manager, we would like to make sure Cosmovisor is automatically restarted if something happens, for example, an error or reboot.
...
Environment="DAEMON_HOME=/home/whispernode/.pylons"
...
[Service]
User=whispernode
ExecStart=/home/whispernode/go/bin/pylonsd start
Restart=always
RestartSec=3
LimitNOFILE=65535
Environment="DAEMON_NAME=pylonsd"
