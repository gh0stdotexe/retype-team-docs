...
```shell
# gitopia cosmovisor-11 daemon
export DAEMON_NAME_11=gitopiad
export DAEMON_HOME_11=$HOME/.gitopia
```
...
sudo chown -R whispernode:whispernode ~/.gitopia/cosmovisor
```
...
Then use the path returned to copy it to the directory Cosmovisor expects. Let's assume the previous command returned `/home/your-user/go/bin/gitopiad`:

```shell
cp $GOPATH/bin/gitopiad ~/.gitopia/cosmovisor/genesis/bin
```
...
Commands sent to Cosmovisor are sent to the underlying binary. For example, `cosmovisor version` is the same as typing `gitopiad version`.

Nevertheless, just as we would manage `gitopiad` using a process manager, we would like to make sure Cosmovisor is automatically restarted if something happens, for example, an error or reboot.

First, create the service file:

```shell
sudo nano /etc/systemd/system/cosmovisor-11.service
```
...
```shell
[Unit]
Description=cosmovisor-11 Gitopia
After=network-online.target
[Service]
User=whispernode
ExecStart=/home/whispernode/go/bin/cosmovisor-11 start
Restart=always
RestartSec=3
LimitNOFILE=4096
Environment="DAEMON_NAME=gitopiad"
Environment="DAEMON_HOME=/home/whispernode/.gitopia"
...
sudo -S systemctl enable cosmovisor-9
sudo systemctl start cosmovisor-9
```

Check it is running using:

```shell
sudo systemctl status cosmovisor-9
```

If you need to monitor the service after launch, you can view the logs using:

```shell
journalctl -u cosmovisor-9 -f
```
