...
```shell
# cosmovisor
export DAEMON_NAME=mantleNode
export DAEMON_HOME=$HOME/.mantleNode
```
...
```shell
mkdir -p ~/.mantleNode/cosmovisor/genesis/bin
mkdir -p ~/.mantleNode/cosmovisor/upgrades
```
...
Then use the path returned to copy it to the directory Cosmovisor expects. Let's assume the previous command returned `/home/your-user/go/bin/mantleNode`:

```shell
cp $GOPATH/bin/mantleNode ~/.mantleNode/cosmovisor/genesis/bin
```
...
Commands sent to Cosmovisor are sent to the underlying binary. For example, `cosmovisor version` is the same as typing `gaiad version`.
...
RestartSec=3
LimitNOFILE=4096
Environment="DAEMON_NAME=mantleNode"
Environment="DAEMON_HOME=/home/whispernode/.mantleNode"
