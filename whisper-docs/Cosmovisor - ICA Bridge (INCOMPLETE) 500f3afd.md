...
```shell
# ICA cosmovisor daemon
export DAEMON_NAME_7=icad
export DAEMON_HOME_7=$HOME/.ica
```
...
```shell
mkdir -p ~/.ica/cosmovisor/genesis/bin
mkdir -p ~/.ica/cosmovisor/upgrades

# Change Ownership of newly created folders (if needed)
sudo chown -R whispernode:whispernode ~/.ica/cosmovisor
```
...
Then use the path returned to copy it to the directory Cosmovisor expects. Let's assume the previous command returned `/home/your-user/go/bin/icad`:

```shell
cp $GOPATH/bin/icad ~/.ica/cosmovisor/genesis/bin
```
...
Commands sent to Cosmovisor are sent to the underlying binary. For example, `cosmovisor version` is the same as typing `icad version`.

Nevertheless, just as we would manage `icad` using a process manager, we would like to make sure Cosmovisor is automatically restarted if something happens, for example, an error or reboot.

First, create the service file:

```shell
sudo nano /etc/systemd/system/cosmovisor-7.service
```
...
```shell
[Unit]
Description=cosmovisor-7
After=network-online.target
[Service]
User=whispernode
ExecStart=/home/whispernode/go/bin/cosmovisor-7 start
Restart=always
RestartSec=3
LimitNOFILE=4096
Environment="DAEMON_NAME=icad"
Environment="DAEMON_HOME=/home/whispernode/.ica"
...
sudo -S systemctl enable cosmovisor-7
sudo systemctl start cosmovisor-7
```

Check it is running using:

```shell
sudo systemctl status cosmovisor-7
```
