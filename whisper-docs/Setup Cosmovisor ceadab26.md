...
mkdir -p ~/.desmos/cosmovisor/upgrades

# Change Ownership of newly created folders
sudo chown -R whispernode:whispernode ~/.desmos/cosmovisor
```
...
Then use the path returned to copy it to the directory Cosmovisor expects. Let's assume the previous command returned `/home/your-user/go/bin/desmos`:

```shell
cp $GOPATH/bin/desmos ~/.desmos/cosmovisor/genesis/bin
```
...
Environment="DAEMON_HOME=/home/whispernode/.desmos"
