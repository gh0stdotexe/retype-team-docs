...
```shell
# kujira cosmovisor daemon
export DAEMON_NAME_5=kujirad
export DAEMON_HOME_5=$HOME/.kujira
```
...
sudo chown -R whispernode:whispernode ~/.kujira/cosmovisor
```
...
```shell
cp $GOPATH/bin/kujirad ~/.kujira/cosmovisor/genesis/bin
```
...
Environment="DAEMON_HOME=/home/whispernode/.kujira"
...
```shell
sudo systemctl status cosmovisor-5
```
